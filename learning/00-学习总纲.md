...
| 文档 | 内容 | 重要度 |
|------|------|--------|
| [01-主干流程](./01-主干流程-从启动到对话循环.md) | 启动 → 对话循环 → API 调用 → 工具执行 → 结果返回 | ★★★★★ |
| [02-四大模块](./02-四大模块-Tool-Prompt-Skill-MCP.md) | Tool / Prompt / Skill / MCP 的设计与实现 | ★★★★★ |
| [03-工程化实践](./03-工程化实践-状态管理-压缩-任务系统.md) | 状态管理 / 上下文压缩 / 工具编排 / QueryEngine | ★★★★☆ |
| [04-System Prompt 构建详解](./04-深入-system-prompt构建详解.md) | prompt 拆分、缓存策略、优先级链、上下文注入 | ★★★★☆ |
| [05-中断机制详解](./05-深入-中断机制详解.md) | AbortController、API 流中断、BashTool 后台化、子 Agent 传播 | ★★★★☆ |
| [06-工具结果注入与 API 调用](./06-深入-工具结果注入与API调用.md) | tool_use 提取、tool_result 生成、消息拼装、完整请求示例 | ★★★★★ |
| [07-上下文窗口解析机制](./07-深入-上下文窗口解析机制.md) | [1M] 后缀原理、6 级优先级、autocompact 阈值、配置错误后果 | ★★★★☆ |
